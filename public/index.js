...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('refresh-images-btn').onclick = () => {
                if (currentArticleData) {
                    ...
                    // Always use the original article data for searching
        ...
        async function fetchNews() {
            ...
            try {
                ...
                articles.forEach(article => {
                    ...
                    // Store the full article object to pass on click
        ...
        // --- STEP 2: Handle Tap & Fan Out API Calls (REWRITTEN) ---
        async function handleArticleTap(article) {
             currentArticleData = article; // Save the original article
             currentCuratedData = null; // Reset curated AI text
             setView('select');
             
             // Reset UI sections to "loading"
             ...
             // Task B: Get Images (using original title and source)
             fetchImages(article.title, article.source);
             
             // Task C: Get Related Articles (using original title and source)
        ...
        async function fetchAiText(article) {
            try {
                ...
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`); // Check for 404/500
        ...
        // --- STEP 3.5: Call Simple Preview API (Unchanged) ---
        ...
        async function handleShare() {
             ...
             if (!currentCuratedData) { alert("AI caption not ready."); return; }
             
             const finalCaption = (currentCuratedData?.caption || "") + `\n\nNews Source: ${currentCuratedData?.originalSource || 'Web Search'}`;
             const platform = prompt("Share to where? (e.g., 'Instagram', 'Twitter')");
             if (!platform) return;
             alert(`Attempting to share to ${platform}... (Check terminal)`);
             try {
                 const response = await fetch('/api/share', { 
                    method: 'POST', 
                    headers: { 'Content-Type': 'application/json' }, 
                    body: JSON.stringify({ imagePath: simplePreviewImagePath, caption: finalCaption, platform: platform })
                 });
                 if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
